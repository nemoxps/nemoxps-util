...
let objReduce = require('lodash/reduce');
let chalk;
try
{
...
class Directory {
    ...
    constructor(dir) {
        ...
        this.size = this.dirs.concat(this.files).reduce((sum, { size }) => sum + size, 0);
    ...
    static Tokenizer(dir, { useColors = false, useOS = false } = {}) {
        ...
        class Tokenizer {
            ...
            toString() {
                return this.tokens.map(String).join('');
            }
        }
    ...
    toString(options) {
        ...
        return Directory.Tokenizer(this, options).toString();
    }
...
let dirTree = async (dirPath, depth = 0, options = {}) => {
    ...
    return (async function dirTree(itemPath, depth, options) {
        ...
        throw new Error('fsp.dirTree: This shouldn\'t happen.');
