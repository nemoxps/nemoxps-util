...
let objMapKeys = require('lodash/mapKeys');
...
test('fsp.dirTree', async (t) => {
    let FIXTURE = path.join(__dirname, 'fsp.dirTree.fixture');
    let fn = dirTree;
    let tt = async (args, expected, msg) => {
        args = [path.join(FIXTURE, args[0]), ...args.slice(1)];
        t.deepEqual(await fn(...args), new Directory(expected), msg);
    };
    tt.default = async (args, defaultArgs, msg) => {
        args = [path.join(FIXTURE, args[0]), ...args.slice(1)];
        defaultArgs = [path.join(FIXTURE, defaultArgs[0]), ...defaultArgs.slice(1)];
    ...
    await tt(['./', 0], {
        ...
        dirs: [
            {
                path: path.join(FIXTURE, 'dir-1'),
                dirs: [],
                files: [],
                isSearched: false,
            },
            {
                path: path.join(FIXTURE, 'dir-2'),
                dirs: [],
                files: [],
                isSearched: false,
            },
        ...
        files: [
            {
                path: path.join(FIXTURE, 'file-1.txt'),
                size: 7,
            },
            {
                path: path.join(FIXTURE, 'file-2.txt'),
                size: 7,
            },
        ],
        isSearched: true,
        ...
        dirs: [
            {
                ...
                dirs: [
                    {
                        path: path.join(FIXTURE, 'dir-1/dir-1-1'),
                        dirs: [],
                        files: [],
                        isSearched: false,
                    },
                    {
                        path: path.join(FIXTURE, 'dir-1/dir-1-2'),
                        dirs: [],
                        files: [],
                        isSearched: false,
                    },
                ...
                files: [
                    {
                        path: path.join(FIXTURE, 'dir-1/file-1-1.txt'),
                        size: 9,
                    },
                ],
                isSearched: true,
            },
            {
                ...
                files: [
                    {
                        path: path.join(FIXTURE, 'dir-2/file-2-1.txt'),
                        size: 9,
                    },
                ],
                isSearched: true,
                ...
                dirs: [
                    {
                        ...
                        files: [
                            {
                                path: path.join(FIXTURE, 'dir-1/dir-1-1/file-1-1-1.txt'),
                                size: 11,
                            },
                        ],
                        isSearched: true,
    ...
    await t.test('fsp.dirTree.from', async (t) => {
        ...
        let tt = async (arg, expected, msg) => {
            arg = objMapKeys(arg, (val, key) => path.join(FIXTURE, key));
